...
def send_email(lad_who_did_the_worst):
    ...
    msg['Subject'] = f'GW {str(lad_who_did_the_worst["gw"])}: not your week eh {lad_who_did_the_worst["name"].split()[0]}?'
    msg['From'] = me
    ...
    if len(lad_who_did_the_worst['transfers']) > 1:
        ...
        for transfer in lad_who_did_the_worst['transfers']:
            html += f'{transfer["out"]} ---> {transfer["in"]}, '
        if lad_who_did_the_worst['hit_cost']:
            html += f' which to remind you cost you {str(lad_who_did_the_worst["hit_cost"])} points, and thus '
        html += f'netted you a total of {str(lad_who_did_the_worst["total_delta"])} points.<br><br>'\
            '<b>Ye good call getting us out you flop!</b><br>'
        for transfer in lad_who_did_the_worst['transfers']:
            html += f'<img src="{transfer["out_photo"]}"/>'
    else:
        transfer = lad_who_did_the_worst['transfers'][0]
        html += f'Your transfer {transfer["out"]} to {transfer["in"]} netted you' \
            f'{str(transfer["delta"])} points.<br><br><b>Ye good call getting me out you flop!</b><br>' \
            f'<img src="{transfer["out_photo"]}"/>'
